...
test(
  "pcb repro27 preserves native text height while fitting silk and fabrication text width",
  async () => {
    ...
    expect(outputString).toMatch(
      /\(gr_text\s+"FAB NOTE"[\s\S]*?\(layer F\.Fab\)[\s\S]*?\(size 0\.8 0\.\d+\)[\s\S]*?\(thickness 0\.08\)/,
    )
