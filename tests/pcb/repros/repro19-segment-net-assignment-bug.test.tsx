...
test("repro19: trace segments lose net assignment when pcb_trace.source_trace_id is a source_net ID and connection_name is absent", () => {
  const circuitJson = [
    {
      type: "pcb_board",
    ...
    },
    {
      ...
      source_trace_id: "net_a", // points to source_net
      route: [
        {
          route_type: "wire",
          x: 0,
  ...
  const segmentMatches = [...output.matchAll(/\(segment\b[\s\S]*?\(net (\d+)\)/g)]
